...
{
    if (!forecastWeatherJsonObject.isEmpty()) {
        // Extract the "list" array, which contains forecast data for different time periods
        QJsonArray forecastList = forecastWeatherJsonObject["list"].toArray();

        // Process each forecast in the list
        for (const QJsonValue &forecastData : forecastList) {
            qint64 timestamp = forecastData.toObject()["dt"].toInt();

            // Extract and print the temperature along with the timestamp
            double temperature = forecastData.toObject()["main"].toObject()["temp"].toDouble();
            qDebug() << "Timestamp:" << timestamp << " - Temperature:" << temperature;

            // Update the UI for each timestamp, e.g., add to a chart or display in a list
            // Example: ui->dailyTemperatureChart->addData(timestamp, temperature);
        }
    } else {
